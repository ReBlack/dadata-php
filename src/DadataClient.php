...
class DadataClient
{
    ...
    {
        $this->cleaner = new CleanClient($token, $secret, $isKeepAlive);
        $this->profile = new ProfileClient($token, $secret, $isKeepAlive);
        $this->suggestions = new SuggestClient($token, $isKeepAlive);
    }
